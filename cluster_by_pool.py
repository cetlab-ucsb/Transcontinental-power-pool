...
cluster=cluster.merge(supergrid,how='left',left_on=['load_zone'],right_on=['ISO3'])
#%%
period=np.arange(2030,2055,10)
horizon=pd.read_csv('C:/Program Files/GRIDPATH/db/csvs_power_pool/temporal/2_global_3period/horizon_params.csv')

timepoints=pd.read_csv('C:/Program Files/GRIDPATH/db/csvs_power_pool/temporal/2_global_3period/structure.csv')['timepoint']

share_path='C:/Program Files/GRIDPATH/db/csvs_power_pool/'

mega_pool=['African super grid',
      'Asian super grid',
      'European super grid',
...
pool_new=pd.read_excel('country_ISO3.xlsx',sheet_name='pool_new')
#%%
#project_portofolio
from project_function import generate_project_portofolio
from project_function import generate_system_load_zone
from transmission_function import generate_transmission_portfolio
from transmission_function import generate_transmission_portfolio_keep
from reliability_function import generate_geography_pool_prm_zones, generate_project_pool_prm_zones, generate_system_prm_pool_req

for i_id, pool in enumerate(mega_pool):
    portfolio_scenario=i_id+30
    portfolio_description='portofolio'+'_'+pool

    path_portfolio=share_path+'project/project_portfolios/'+str(portfolio_scenario)+'_'+portfolio_description+'.csv'
    
    power_pool=cluster.loc[cluster.pool==pool,:]
    
    project_portofolio=generate_project_portofolio(power_pool)
    if output=='yes':
        project_portofolio.to_csv(path_portfolio,index=False)
    
    
    load_zones=generate_system_load_zone(power_pool)
    load_zones.to_csv(share_path+'system_load/load_zones/'+str(portfolio_scenario)+'_'+i+'.csv',index=False) 
    
    '''
    transmission_portfolio=generate_transmission_portfolio([pool])
    if pool in ['African super grid','European super grid','South American super grid']:
        transmission_portfolio=generate_transmission_portfolio_keep(pool_new,transmission_portfolio,pool) 
    transmission_portfolio.to_csv(share_path+'transmission/transmission_portfolios/'+str(portfolio_scenario+1)+'_'+pool+'_all'+'.csv',index=False)
    '''
    reliability_scenario=i_id+8
    #geography_pool_prm_zones=generate_geography_pool_prm_zones(power_pool)  
    #geography_pool_prm_zones.to_csv(share_path+'reliability/prm/geography_prm_zones/'+str(reliability_scenario+1)+'_'+pool+'.csv',index=False)
    #project_pool_prm_zones=generate_project_pool_prm_zones(power_pool)
    #project_pool_prm_zones.to_csv(share_path+'reliability/prm/project_prm_zones/'+str(reliability_scenario+1)+'_'+pool+'.csv',index=False)
    scenario=['IEA_NZE']
    generate_system_prm_pool_req(scenario,reliability_scenario,share_path,pool_new,pool)
...
from project_function import generate_hydro_operation_chars
hydro_oper_scenario_id=1
project_hydro_operational_chars=generate_hydro_operation_chars(cluster,hydro_oper_scenario_id,horizon,share_path)
...
#%%project_variable
from project_function import generate_variable_generator_profile
...
file_var=generate_variable_generator_profile(cluster, variable_scenario_id, description, timepoints, share_path)
...
from project_function import generate_system_load
scenario=['IEA_SDG']
demand=pd.read_excel('country_ISO3.xlsx',sheet_name='demand')

generate_system_load(demand,scenario,timepoints,share_path)

#%% prm
from reliability_function import generate_geography_prm_zones
geography_prm_zones=generate_geography_prm_zones(cluster)
geography_prm_zones.to_csv(share_path+'reliability/prm/geography_prm_zones/1_geography_prm_zones.csv',index=False)

from reliability_function import generate_project_prm_zones
project_prm_zones=generate_project_prm_zones(cluster)
project_prm_zones.to_csv(share_path+'reliability/prm/project_prm_zones/1_project_prm_zones.csv',index=False)


from reliability_function import generate_prm_project_elcc
#project_elcc_chars
prm_project_elcc=generate_prm_project_elcc(cluster)
prm_project_elcc.to_csv(share_path+'reliability/prm/project_elcc_chars/1_project_elcc.csv',index=False)

#system prm requirement
from reliability_function import generate_system_prm_req

generate_system_prm_req(scenario,share_path)
...
from transmission_function import generate_transmission_portfolio
transmission_portfolio=generate_transmission_portfolio(pool)
transmission_portfolio.to_csv(share_path+'transmission/transmission_portfolios/2_transmission_porfolio_pool.csv',index=False)
